...
- Rate-limit classes: the pinned ingest budget (120/min, burst 240) applies to `POST /v1/ingest` (metric batches); run-lifecycle calls (register/heartbeat/ack-kill/complete) use the default 300/min class — otherwise 8 demo runs on one key starve their own heartbeats and kill directives never get acknowledged.
